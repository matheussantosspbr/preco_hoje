...
export default function Home() {
  ...
  const classes = {
    input:
      "text-lg outline-none dark:bg-gray-900 bg-[#dedede] placeholder:text-[#909090] text-[#666666] dark:text-white rounded-r w-[36rem] 2xl:w-[28rem] xs:!w-[10rem] h-16 p-2 pt-0 pb-0 flex items-center justify-start  border border-l-0 border-black dark:border-cyan-500 rounded-r",
    select:
      "text-lg w-24 h-16 text-[#666666] dark:text-white bg-[#dedede] dark:bg-gray-900 border border-black dark:border-cyan-500 rounded-l outline-none pl-2",
  };
  ...
  const calcular = () => {
    ...
    let valorFormatado =
      moeda1 == "USD"
        ? valorParaFormatar.replace(/,/g, "")
        : valorParaFormatar.split(".").join("");
    let valorComPonto =
      moeda1 != "USD" ? valorFormatado.replace(",", ".") : valorFormatado;

    if (moeda1 == "BRL" && moeda2 == "USD") {
      setResult(
        (parseFloat(valorComPonto) * BRL_USD)
          .toLocaleString("en", { style: "currency", currency: "USD" })
          .toString()
      );
    } else if (moeda1 == "USD" && moeda2 == "BRL") {
      setResult(
        (parseFloat(valorComPonto) * USD_BRL)
          .toLocaleString("pt-BR", { style: "currency", currency: "BRL" })
          .toString()
      );
    } else if (moeda1 == "BRL" && moeda2 == "EUR") {
      setResult(
        (parseFloat(valorComPonto) * BRL_EUR)
          .toLocaleString("de-DE", { style: "currency", currency: "EUR" })
          .toString()
      );
    } else if (moeda1 == "EUR" && moeda2 == "BRL") {
      setResult(
        (parseFloat(valorComPonto) * EUR_BRL)
          .toLocaleString("pt-BR", { style: "currency", currency: "BRL" })
          .toString()
      );
    } else if (moeda1 == "EUR" && moeda2 == "USD") {
      setResult(
        (parseFloat(valorComPonto) * EUR_USD)
          .toLocaleString("en", { style: "currency", currency: "USD" })
          .toString()
      );
    } else if (moeda1 == "USD" && moeda2 == "EUR") {
      setResult(
        (parseFloat(valorComPonto) * USD_EUR)
          .toLocaleString("de-DE", { style: "currency", currency: "EUR" })
          .toString()
      );
    }
  ...
  return (
    <div className="flex items-center justify-center flex-col h-auto w-full">
      ...
      <div className="h-[32rem] flex items-center justify-evenly flex-row w-full xl:flex-col xl:justify-center">
        ...
        <i
          className="fa-solid fa-arrows-rotate text-4xl text-gray-900 dark:text-white xl:rotate-90 xl:m-10 md:text-3xl cursor-pointer"
      ...
      <div>
        <script
          async
          src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-1617465550802261"
